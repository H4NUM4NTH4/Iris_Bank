...
def train_model(embeddings, labels):
    ...
    # Train the model
    model = SVC(kernel='linear', C=0.5, probability=True, class_weight='balanced')
...
def verify_template(args):
    ...
    if not basename.startswith("300_1_"):  # Check if filename starts with "300_1_"
        print(f"Invalid filename format for {args.filename}. Verification failed.\n")
        return False

    template_file = os.path.join(args.template_dir, f"{basename}_template.npy")
    if os.path.exists(template_file):
        print(f"Template found for {args.filename}. Proceeding with verification.")
        ...
        # If template doesn't exist, return failure
        print(f"No template found for {args.filename}. Verification failed.\n")
        return False

def main():
    # Argument parser
    parser = argparse.ArgumentParser(description="Train model or verify image against templates.")
    parser.add_argument("--template_dir", type=str, default="F:/MAJOR_PROJECT/IrisRecognition_ML/src/templates/CASIA1/",
